...
async function initAudioWorkletPlayer(ctx) {
  ...
  if (!window.crossOriginIsolated || typeof SharedArrayBuffer === "undefined") {
    log("AUDIO: SharedArrayBuffer unavailable — skipping AudioWorklet ring.", "warn");
    return;
  }
  ...
  (async () => {
    ...
    try {
      if (typeof SharedArrayBuffer === 'undefined' || !window.crossOriginIsolated) {
        log('AUDIO: SAB unavailable — leaving native ScriptProcessorNode active.', 'warn');
        return;
      }
...
async function installScriptProcessorPolyfill(ctx) {
  // SAB is optional. Without it, keep the browser's native
  // ScriptProcessorNode instead of installing the SAB-based worklet shim.
  if (!ctx || !ctx.audioWorklet) {
    log('AUDIO: AudioWorklet not available — using native ScriptProcessor.', 'warn');
    return;
  }
  if (typeof SharedArrayBuffer === 'undefined' || !window.crossOriginIsolated) {
    log('AUDIO: SharedArrayBuffer unavailable — using native ScriptProcessor fallback.', 'warn');
    return;
  }
...
// SharedArrayBuffer is optional. No COOP/COEP or service-worker reload is required.
// A threaded PPSSPP WASM build still needs SAB internally; use a non-threaded build
// if the compiled runtime itself was built with Emscripten pthreads.
...
async function preloadGame(FS) {
  ...
  // Note: WORKERFS fast-mount is skipped for OPFS-stored games — this build's
  // WORKERFS support reliably aborts on OPFS-sourced File handles, so we go
  // straight to the MEMFS path below instead of eating a guaranteed abort first.
...
async function start() {
  ...
  // SharedArrayBuffer is optional. The runtime can use the normal
  // single-threaded Emscripten build and native ScriptProcessor audio path.
  started = true;
  ...
  let gameArg = null;
  let hadPreInitAbort = false; // set if Module.onAbort fires before the runtime is ever ready (e.g. a caught, recoverable WORKERFS failure)
  ...
  window.Module = {
    ...
    onRuntimeInitialized() {
      log("Runtime initialized." + (hadPreInitAbort ? " (recovered after an earlier non-fatal abort)" : ""), "ok");
      runtimeReady = true;
    ...
    onAbort(reason) {
      if (runtimeReady) {
        // Genuine fatal abort of an already-running emulator.
        log("Runtime abort: " + reason, "err");
        setStatus("Abort: " + reason, "err");
        runtimeReady = false;
        hideLoading();
      } else {
        // Emscripten's abort() can also fire for recoverable, JS-level
        // failures before the runtime has ever finished initializing (e.g.
        // an unsupported WORKERFS mount) — these are caught by their
        // callers and initialization continues normally afterward, so
        // don't clobber the UI with a red error or a false "not ready" state.
        hadPreInitAbort = true;
        log("Non-fatal early abort (reason: " + reason + "), continuing initialization…", "warn");
      }
    }
  };
