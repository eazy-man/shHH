...
class IsoDomItem {
     ...
     * @param orientation
     */
    rotate(clockwise = true, orientation) {
        ...
        const newOrientation = orientation || this.iso.config.orientationOrder[next];

        this.orientate(newOrientation);
        this.update();
...
class IsoDom {
        ...
        for (let y = row; y < this.config.rows; y++) {
            for (let x = col; x < this.config.columns; x++) {
                ...
                if (cell.isItemRoot()) {
                    ...
                    this._renderPath(x, y, cell.item.name);
                    break;
                    ...
                    this._renderPath(root.x, root.y, cell.item.name);
     ...
     * * @param {boolean} suppressAssert
     * @returns {IsoDomItem}
     */
    addItem(item, x, y, config = null, suppressAssert = false) {
        let isoItem = item;
    ...
    /**
     * Add ghost item to the grid. This item does not have a cell attached to it, call moveItem to attach it.
     * @param {IsoDomItem|string} item
     * @param {Object.<string, *>} config Config value for IsoDomItem if constructing by item name.
     * @returns {IsoDomItem}
     */
    addGhostItem(item, config = null) {
        let isoItem = item;
        if (typeof item === 'string') {
            isoItem = this.makeItem(item, config);
        }

        this.emit('itemAdded', isoItem, null, this);
     ...
     * @param {boolean} suppressAssert
     */
    moveItem(item, x, y, suppressAssert = false) {
        if (!item.cell) {
            this.addItem(item, x, y, null, false);
            return;
        }
